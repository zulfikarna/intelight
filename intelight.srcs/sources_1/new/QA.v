...
module QA(
    ...
    wire [31:0] w_curr_qA0, w_curr_qA1, w_curr_qA2, w_curr_qA3;
    ...
    reg_32bit reg0(.in0(next_qA0), .out0(w_curr_qA0), .clk(clk));
    reg_32bit reg1(.in0(next_qA1), .out0(w_curr_qA1), .clk(clk));
    reg_32bit reg2(.in0(next_qA2), .out0(w_curr_qA2), .clk(clk));
    reg_32bit reg3(.in0(next_qA3), .out0(w_curr_qA3), .clk(clk));
    assign curr_qA0 = w_curr_qA0;
    assign curr_qA1 = w_curr_qA1;
    assign curr_qA2 = w_curr_qA2;
    assign curr_qA3 = w_curr_qA3;
    max4to1_32bit max0 (.in0(next_qA0), .in1(next_qA1), .in2(next_qA2), .in3(next_qA3), .out0(w_max_next_qA));
    assign max_next_qA = w_max_next_qA;
    mux4to1_32bit mux0 (.in0(w_curr_qA0), .in1(w_curr_qA1), .in2(w_curr_qA2), .in3(w_curr_qA3), .out0(chos_curr_qA), .sel(act));
    
    //  Implementation of Optimezed-Bellman Equat ion
    //  Qnew = Q + alpha(reward+gamma(maxQ')-Q)
    // 1. gamma(maxQ')
    multiply mult_gamma(.in0(w_max_next_qA),    .c(gamma), .out0(o_gamma));
